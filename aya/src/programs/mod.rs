...
pub enum Program {
    ...
    /// A [`StructOps`] program
    StructOps(StructOps),
    /// A [`SyscallProgram`] program
    SyscallProgram(SyscallProgram),
}
...
impl Program {
    ...
    pub const fn prog_type(&self) -> ProgramType {
        match self {
            ...
            Self::SyscallProgram(_) => ProgramType::Syscall,
        }
    }
    ...
    pub fn pin<P: AsRef<Path>>(&mut self, path: P) -> Result<(), PinError> {
        match self {
            ...
            Self::SyscallProgram(p) => p.pin(path),
        }
    }
    ...
    pub fn unload(self) -> Result<(), ProgramError> {
        match self {
            ...
            Self::SyscallProgram(mut p) => p.unload(),
        }
    }
